...
=== 0.1.0
* **BREAKING CHANGES**
  * Previously the ruby_odata `Service` used to return a single object if there was only one result returned. Now, the results are always an Enumerable, so if you just need one result, use the `first` method on the result set
* New Features
