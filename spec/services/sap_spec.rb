    ...
    describe "handling of SAP results" do
      before(:each) do
        ...
        stub_request(:get, "http://test.com/test.svc/$metadata").
          ...
          to_return(:status => 200, :body => File.new( FIXTURES + "/sap/edmx_sap_demo_flight.xml"), :headers => {})

        stub_request(:get, "http://test.com/test.svc/z_demo_flightCollection").
          with(:headers => DEFAULT_HEADERS).
          to_return(:status => 200, :body => File.new( FIXTURES + "/sap/result_sap_demo_flight_missing_category.xml"), :headers => {})
      end
